...
3. `collect.yml` uses a fresh GitHub-hosted `windows-2022` runner, so no local self-hosted runner installation is required. Add `CME_EMAIL` and `CME_PASSWORD` as repository secrets because the temporary runner must authenticate to CME on every OI run.
